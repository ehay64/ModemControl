...
void doexecute_lite_ims_test(testitem_t *pImsTestParams);
extern unsigned g_xid;
int ims_fd = -1;
extern testitem_t imstotest[];
extern unsigned int imsarraylen;

void display_ims_test_list()
{
    unsigned int i;
    ...
    for (i = 0; i < imsarraylen; i++)
    {
        strcpy(test,"t");
        sprintf(testid, "%u", i);
        strcat(test,testid);
        printf ("%4s\t%s \n", test, &imstotest[i].unpack_func_name[7]);
    }
}
void run_ims_tests()
{
    ...
    while(1) {
        ...
        printf ("Invoking test: %s\n", &imstotest[id].unpack_func_name[7]);
        doexecute_lite_ims_test(&imstotest[id]);            
    }
}

void *ims_test_read_thread(void* ptr)
{
    ...
    while(1)
    {
        if (ims_fd < 0)
            break;
        rtn = read(ims_fd, rsp, QMI_MSG_MAX);
        if (rtn > 0)
        {
            rspLen = (uint16_t)rtn;
            helper_get_resp_ctx(eIMS, rsp, rspLen, &rsp_ctx);
            #ifdef QMI_DBG      
            int j;
            printf("<< receiving QMI ims \n");
            for (j = 0; j < rspLen; j++)
            {
                printf("%02x ", rsp[j]);
                if ((j+1)%16 == 0)
        ...
        }else
        {
           /* break the thread */
           #ifdef TEST_DBG
           fprintf(stderr,"ims read thread error\n");
           #endif
           break;
        }
    }
...
void doexecute_lite_ims_test(testitem_t *pImsTestParams)
{
    ...
    /* open ims descriptor */
    ims_fd = client_fd(eIMS);

    if (ims_fd < 0) {
        fprintf( stderr, "%s: ims svc could not open!\n", __func__ );
        return;
    }

    pthread_attr_init(&ims_attr);

    /* create ims read thread */
    if ((pthread_create(&ims_tid, &ims_attr, ims_test_read_thread, (void*)pImsTestParams)) < 0) {
        fprintf( stderr, "%s: ims read thread not created!\n", __func__ );
        close(ims_fd);
        ims_fd = -1;
        pthread_attr_destroy(&ims_attr);
        return;
    }
    ...
    if (pImsTestParams->pack) { 
        rtn = run_pack_item(pImsTestParams->pack)(&req_ctx, req, &reqLen,pImsTestParams->pack_ptr);
        if(rtn !=eQCWWAN_ERR_NONE)
        {
            close(ims_fd);
            ims_fd = -1;
            fprintf (stderr, "packing function %s returned error %d\n", 
                      pImsTestParams->pack_func_name,
                      rtn);
            goto finish;
        }
    } else {
...
finish:
    /* Wait for ims read thread termination */
    pthread_join(ims_tid, NULL);
    if (ims_fd > 0) {
        close(ims_fd);
        ims_fd = -1;
    }
