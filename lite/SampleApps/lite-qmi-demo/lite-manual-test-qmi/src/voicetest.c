...
void doexecute_lite_voice_test(testitem_t *pVoiceTestParams);
extern unsigned g_xid;
int voice_fd = -1;
extern testitem_t voicetotest[];
extern unsigned int voicearraylen;

void display_voice_test_list()
{
    unsigned int i;
    ...
    for (i = 0; i < voicearraylen; i++)
    {
        strcpy(test,"t");
        sprintf(testid, "%u", i);
        strcat(test,testid);
        printf ("%4s\t%s \n", test, &voicetotest[i].unpack_func_name[7]);
    }
}
void run_voice_tests()
{
    ...
    while(1) {
        ...
        printf ("Invoking test: %s\n", &voicetotest[id].unpack_func_name[7]);
        doexecute_lite_voice_test(&voicetotest[id]);            
    }
}

void *voice_test_read_thread(void* ptr)
{
    ...
    while(1)
    {
        if (voice_fd < 0)
            break;
        rtn = read(voice_fd, rsp, QMI_MSG_MAX);
        if (rtn > 0)
        {
            rspLen = (uint16_t)rtn;
            helper_get_resp_ctx(eVOICE, rsp, rspLen, &rsp_ctx);
            #ifdef QMI_DBG      
            int j;
            printf("<< receiving QMI voice \n");
            for (j = 0; j < rspLen; j++)
            {
                printf("%02x ", rsp[j]);
                if ((j+1)%16 == 0)
        ...
        }else
        {
           /* break the thread */
           #ifdef TEST_DBG
           fprintf(stderr,"voice read thread error\n");
           #endif
           break;
        }
    }
...
void doexecute_lite_voice_test(testitem_t *pVoiceTestParams)
{
    ...
    /* open voice descriptor */
    voice_fd = client_fd(eVOICE);

    if (voice_fd < 0) {
        fprintf( stderr, "%s: voice svc could not open!\n", __func__ );
        return;
    }

    pthread_attr_init(&voice_attr);

    /* create voice read thread */
    if ((pthread_create(&voice_tid, &voice_attr, voice_test_read_thread, (void*)pVoiceTestParams)) < 0) {
        fprintf( stderr, "%s: voice read thread not created!\n", __func__ );
        close(voice_fd);
        voice_fd = -1;
        pthread_attr_destroy(&voice_attr);
        return;
    }
    ...
    if (pVoiceTestParams->pack) { 
        rtn = run_pack_item(pVoiceTestParams->pack)(&req_ctx, req, &reqLen,pVoiceTestParams->pack_ptr);
        if(rtn !=eQCWWAN_ERR_NONE)
        {
            close(voice_fd);
            voice_fd = -1;
            fprintf (stderr, "packing function %s returned error %d\n", 
                      pVoiceTestParams->pack_func_name,
                      rtn);
            goto finish;
        }
    } else {
...
finish:
    /* Wait for voice read thread termination */
