...
void doexecute_lite_swiaudio_test(testitem_t *pSwiaudioTestParams);
extern unsigned g_xid;
int swiaudio_fd = -1;
extern testitem_t swiaudiototest[];
extern unsigned int swiaudioarraylen;

void display_swiaudio_test_list()
{
    unsigned int i;
    ...
    printf("\n\t=================================");
    printf("\tSWIAUDIO APIs");
    printf("\t===================================\n\n");
    printf("Test id API name\n");
    printf("======= ========\n");

    for (i = 0; i < swiaudioarraylen; i++)
    {
        strcpy(test,"t");
        sprintf(testid, "%u", i);
        strcat(test,testid);
        printf ("%4s\t%s \n", test, &swiaudiototest[i].unpack_func_name[7]);
    }
}
void run_swiaudio_tests()
{
    ...
    while(1) {
        ...
        if (id >= swiaudioarraylen) {
            printf("\nTest case id is not valid \n");
            continue;
        }

        /* Invoke the selected API function */
        printf ("Invoking test: %s\n", &swiaudiototest[id].unpack_func_name[7]);
        doexecute_lite_swiaudio_test(&swiaudiototest[id]);            
    }
}

void *swiaudio_test_read_thread(void* ptr)
{
    ...
    while(1)
    {
        if (swiaudio_fd < 0)
            break;
        rtn = read(swiaudio_fd, rsp, QMI_MSG_MAX);
        if (rtn > 0)
        {
            rspLen = (uint16_t)rtn;
            helper_get_resp_ctx(eSWIAUDIO, rsp, rspLen, &rsp_ctx);
            #ifdef QMI_DBG      
            int j;
            printf("<< receiving QMI swiaudio \n");
            for (j = 0; j < rspLen; j++)
            {
                printf("%02x ", rsp[j]);
                if ((j+1)%16 == 0)
        ...
        }else
        {
           /* break the thread */
           #ifdef TEST_DBG
           fprintf(stderr,"swiaudio read thread error\n");
           #endif
           break;
        }
    }
...
void doexecute_lite_swiaudio_test(testitem_t *pSwiaudioTestParams)
{
    ...
    /* open swiaudio descriptor */
    swiaudio_fd = client_fd(eSWIAUDIO);

    if (swiaudio_fd < 0) {
        fprintf( stderr, "%s: swiaudio svc could not open!\n", __func__ );
        return;
    }

    pthread_attr_init(&swiaudio_attr);

    /* create swiaudio read thread */
    if ((pthread_create(&swiaudio_tid, &swiaudio_attr, swiaudio_test_read_thread, (void*)pSwiaudioTestParams)) < 0) {
        fprintf( stderr, "%s: swiaudio read thread not created!\n", __func__ );
        close(swiaudio_fd);
        swiaudio_fd = -1;
        pthread_attr_destroy(&swiaudio_attr);
        return;
    }
    ...
    if (pSwiaudioTestParams->pack) { 
        rtn = run_pack_item(pSwiaudioTestParams->pack)(&req_ctx, req, &reqLen,pSwiaudioTestParams->pack_ptr);
        if(rtn !=eQCWWAN_ERR_NONE)
        {
            close(swiaudio_fd);
            swiaudio_fd = -1;
            fprintf (stderr, "packing function %s returned error %d\n", 
                      pSwiaudioTestParams->pack_func_name,
                      rtn);
            goto finish;
        }
    } else {
    ...
    rtn = write(swiaudio_fd, req, reqLen);
    if (rtn!=reqLen)
    {
       close(swiaudio_fd);
       swiaudio_fd = -1;
...
finish:
    /* Wait for swiaudio read thread termination */
    pthread_join(swiaudio_tid, NULL);
    if (swiaudio_fd > 0) {
        close(swiaudio_fd);
        swiaudio_fd = -1;
    }
