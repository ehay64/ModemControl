...
void doexecute_lite_dms_test(testitem_t *pDmsTestParams);
extern unsigned g_xid;
int dms_fd = -1;
extern testitem_t dmstotest[];
extern unsigned int dmsarraylen;

void display_dms_test_list()
{
    unsigned int i;
    ...
    printf("\n\t==================================");
    printf("\tDevice Management Service (DMS) APIs");
    printf("\t====================================\n\n");
    printf("Test id API name\n");
    printf("======= ========\n");

    for (i = 0; i < dmsarraylen; i++)
    {
        strcpy(test,"t");
        sprintf(testid, "%u", i);
        strcat(test,testid);
        printf ("%4s\t%s \n", test, &dmstotest[i].unpack_func_name[7]);
    }
}
void run_dms_tests()
{
    ...
    while(1) {
        ...
        printf ("Invoking test: %s\n", &dmstotest[id].unpack_func_name[7]);
        doexecute_lite_dms_test(&dmstotest[id]);            
    }
}

void *dms_test_read_thread(void* ptr)
{
    ...
    while(1)
    {
        if (dms_fd < 0)
            break;
        rtn = read(dms_fd, rsp, QMI_MSG_MAX);
        if (rtn > 0)
        {
            rspLen = (uint16_t)rtn;
        ...
        }else
        {
           /* break the thread */
           #ifdef TEST_DBG
           fprintf(stderr,"dms read thread error\n");
           #endif
           break;
        }
    }
...
void doexecute_lite_dms_test(testitem_t *pDmsTestParams)
{
    ...
    /* open dms descriptor */
    dms_fd = client_fd(eDMS);

    if (dms_fd < 0) {
        fprintf( stderr, "%s: dms svc could not open!\n", __func__ );
        return;
    }

    pthread_attr_init(&dms_attr);

    /* create dms read thread */
    if ((pthread_create(&dms_tid, &dms_attr, dms_test_read_thread, (void*)pDmsTestParams)) < 0) {
        fprintf( stderr, "%s: dms read thread not created!\n", __func__ );
        close(dms_fd);
        dms_fd = -1;
        pthread_attr_destroy(&dms_attr);
        return;
    }
    ...
    if (pDmsTestParams->pack) { 
        rtn = run_pack_item(pDmsTestParams->pack)(&req_ctx, req, &reqLen,pDmsTestParams->pack_ptr);
        if(rtn !=eQCWWAN_ERR_NONE)
        {
            close(dms_fd);
            dms_fd = -1;
            fprintf (stderr, "packing function %s returned error %d\n", 
                      pDmsTestParams->pack_func_name,
                      rtn);
            goto finish;
        }
    } else {
...
finish:
    /* Wait for dms read thread termination */
    pthread_join(dms_tid, NULL);
    if (dms_fd > 0) {
        close(dms_fd);
        dms_fd = -1;
    }
